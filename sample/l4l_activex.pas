...
begin
  ...
  if TVarData(p^).vtype <> varDispatch then
    begin
      //key := LowerCase(key);
      if key = 'value' then
        begin
...
begin
  ...
  if TVarData(p^).vtype <> varDispatch then
    ...
    begin
      id := IDispatch(TVarData(p^).vDispatch);
      ws := UTF8Decode(key);
      if lua_isfunction(L, 3) or lua_isnil(L, 3) then
        begin
          // Set event
          lua_getmetatable(L, 1);
...
{$ENDIF}
  ...
  case VarType(ret) of
    ...
    varlongword,
    varint64,
    varqword: lua_pushinteger(L, ret);
    varSingle,
    varDouble,
    vardecimal: lua_pushnumber(L, ret);
    varBoolean: lua_pushboolean(L, ret);
    varDispatch: DoCreateActiveXObject(L, ret);
    else
      begin
...
begin
  ...
  if TVarData(p1^).vtype <> varDispatch then
    Exit;
  if IDispatch(TVarData(p1^).vdispatch).GetTypeInfo(0, 0, ti1) <> S_OK then
    Exit;
  if ti1.GetTypeAttr(ta1) <> S_OK then
    Exit;
  try
    lua_getmetatable(L, 1);
    lua_getfield(L, -1, FIELD_ID);
    ...
    lua_pop(L, 2);
    if TVarData(p2^).vtype <> varDispatch then
      Exit;
    if IDispatch(TVarData(p2^).vdispatch).GetTypeInfo(0, 0, ti2) <> S_OK then
      Exit;
    if ti2.GetTypeAttr(ta2) <> S_OK then
      Exit;
    try
      if IsEqualIID(ta1^.GUID, ta2^.GUID) then
      begin
        lua_pop(L, 1);
...
begin
  ...
  if id.Invoke(DISPID_VALUE, GUID_NULL,
      GetUserDefaultLCID,
      DISPATCH_PROPERTYGET or DISPATCH_METHOD,
      param, @ret, nil, nil) = 0 then
    begin
      case VarType(ret) of
        varNull: lua_pushnil(L);
        varSmallint,
        ...
        varBoolean: lua_pushboolean(L, ret);
        varDispatch:  begin
                        if TVarData(ret).vdispatch <> nil then
                          begin
...
begin
  Result := S_FALSE;
  if FInfo.GetDocumentation(DispId, @name, nil, nil, nil) <> S_OK then
    Exit;
  if name = '' then
    Exit;
  ...
  begin
    ...
    begin
      ...
      case VarType(v) of
        ...
        varlongword,
        varint64,
        varqword: lua_pushinteger(L, v);
        varSingle,
        varDouble,
        vardecimal: lua_pushnumber(L, v);
...
begin
  ...
  if Assigned(enum) then
    begin
      enum.Reset;
      i := 0;
      while enum.Next(1, cp, @fetched) = S_OK do
        begin
          if fetched = 1 then
            begin
              if cp.GetConnectionInterface(iid) <> S_OK then
                continue;
              if server.GetTypeInfo(0, 0, ti) <> S_OK then
                continue;
              if ti.GetContainingTypeLib(tl, num) <> S_OK then
                continue;
              if tl.GetTypeInfoOfGuid(iid, FInfo) <> S_OK then
              continue;
              if cp.Advise(Self, FCookie) <> S_OK then
                continue;
              FCPoint := cp;
            end;
          Inc(i);
